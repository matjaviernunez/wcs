...
for(i in 1:length(archivos)){
  ...
  for(j in 1:length(dicc)){
    ...
    vat1 <- import(archivos[i], sheet = dicc[j]) %>% 
      filter(`Formato de Datos` == "Categórica") %>% 
      select(nom_act = `Nombre propuesto`)
    vat1 <- vat1[,1]
    ...
    for(k in 1:length(pp)){
      ...
      # añadir hoja
      addWorksheet(wb, substr(paste(pp1[k], nh, sep = "_"), 1, 30))
      ...
      writeData(wb, sheet = substr(paste(pp1[k], nh, sep = "_"), 1, 30)
        , x = hh)
